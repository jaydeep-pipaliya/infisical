...
describe("Identity Access Token — redesigned JWT flow", () => {
  ...
  test("issuance with accessTokenTTL=0 caps exp to MAX_MACHINE_IDENTITY_TOKEN_AGE", async () => {
    ...
    try {
      ...
      expect(issuedTtl).toBeLessThanOrEqual(appCfg.MAX_MACHINE_IDENTITY_TOKEN_AGE);
      expect(issuedTtl).toBeGreaterThanOrEqual(appCfg.MAX_MACHINE_IDENTITY_TOKEN_AGE - 2);
    } finally {
  ...
  test("legacy JWT missing jti and orgId is rejected with 401", async () => {
    ...
    const legacyToken = jwt.sign(legacyPayload, appCfg.AUTH_SECRET, { expiresIn: 3600 });
