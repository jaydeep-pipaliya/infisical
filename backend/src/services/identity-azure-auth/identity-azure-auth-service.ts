  ...
  const login = async ({ identityId, jwt: azureJwt, organizationSlug }: TLoginAzureAuthDTO) => {
    ...
    try {
      ...
      const { accessToken, identityAccessToken } = await identityAccessTokenService.issueIdentityAccessToken({
        ...
        subOrganizationId,
        accessTokenTTL: Number(identityAzureAuth.accessTokenTTL),
        accessTokenMaxTTL: Number(identityAzureAuth.accessTokenMaxTTL),
        accessTokenNumUsesLimit: Number(identityAzureAuth.accessTokenNumUsesLimit),
        accessTokenPeriod: Number(identityAzureAuth.accessTokenPeriod) || 0,
        accessTokenTrustedIps: identityAzureAuth.accessTokenTrustedIps as TIp[]
      });
